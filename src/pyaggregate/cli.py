...
def init_db(
    ...
    try:
        ...
        store = CatalogStore(cfg.state.catalog_db)
        store.init_schema()
        store.close()
...
def show_scans(
    ...
    try:
        ...
        if not rows:
            ...
            # Print rows
            for row in rows:
                typer.echo(" | ".join(str(v) if v is not None else "NULL" for v in row))
