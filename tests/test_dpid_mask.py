...
class TestMaskDpidBasic:
    ...
    def test_mask_dpid_with_known_mappings(self, dpid_map_fixture: pl.DataFrame) -> None:
        """Test masking with known dp -> surrogate_id mappings."""
        frame = pl.DataFrame(
            {
                "dp": ["aeos", "cms", "aeos"],
                "patient_id": [1, 2, 3],
        ...
        assert "dp" not in result.columns
    ...
    def test_mask_dpid_preserves_other_columns(self, dpid_map_fixture: pl.DataFrame) -> None:
        """Test that masking preserves columns other than dp."""
        frame = pl.DataFrame(
            {
                "dp": ["aeos", "cms"],
                "patient_id": [1, 2],
...
class TestMaskDpidProperties:
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_no_dp_column(
        self, data: list[tuple[str, int]], dpid_map_fixture: pl.DataFrame
    ) -> None:
        """Property: Result has no dp column."""
        frame = pl.DataFrame(
            {
                "dp": [dp for dp, _ in data],
                "value": [val for _, val in data],
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_surrogate_uniqueness_per_dp(
        self, data: list[tuple[str, int]], dpid_map_fixture: pl.DataFrame
    ) -> None:
        """Property: Each unique dp maps to exactly one unique surrogate_id (injective mapping)."""
        frame = pl.DataFrame(
            {
                "dp": [dp for dp, _ in data],
                "value": [val for _, val in data],
        ...
        # Verify the mapping property: use the dpid_map to check each unique dp in the original
        # maps to exactly one surrogate_id
        dp_to_surrogate = {}
        for row in frame.iter_rows(named=True):
            dp = row["dp"]
            # Find surrogate in dpid_map (catalog column is named `dpid`)
            matching = dpid_map_fixture.filter(pl.col("dpid") == dp)
            if len(matching) > 0:
                surrogate = matching["surrogate_id"][0]
                if dp not in dp_to_surrogate:
                    dp_to_surrogate[dp] = surrogate
                else:
                    # Verify it's the same surrogate (consistency)
                    assert dp_to_surrogate[dp] == surrogate

        surrogates = list(dp_to_surrogate.values())
        assert len(surrogates) == len(set(surrogates)), (
            f"Mapping is not injective: {dp_to_surrogate}"
        )

        for _orig_dp, expected_surrogate in dp_to_surrogate.items():
    ...
    def test_unmapped_dp_produces_null(self, dpid_map_fixture: pl.DataFrame) -> None:
        """Test that unmapped dp produces null surrogate_id."""
        frame = pl.DataFrame(
            {
                "dp": ["unknown_dp"],
                "value": [1],
